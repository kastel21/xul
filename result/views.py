...
def home(request):
    ...
    courses1 = Course.objects.filter(owner=request.user).all().count()
    no_of_carry_over_students = CarryOverStudent.objects.all().count()
    no_of_students_to_repeat = RepeatingStudent.objects.all().count()
    registered = Reg.objects.filter( owner = request.user )


    is_reg = registered[4]
    institute = registered[2]
    date = registered[5]

    #no_of_courses4owner = courses1[]
...
def submit_update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Your document was successfully uploaded.')
            return redirect("/profile/")
    else:
        messages.success(request, 'Your document was not successfully uploaded try again.')

    return render(request, 'account/update_document.html', {'form': form})
...
def report_list(request):
    ...
    return render(request, 'course/report_list.html', context)
...
class RegAddView(CreateView):
    model = Reg
    form_class = RegAddForm
    template_name = 'registration/reg_add.html'
