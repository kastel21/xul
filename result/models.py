...
class Doc(models.Model):
    ...
    doc = models.ImageField(upload_to="pictures/", blank=True, null=True)
...
class Report(models.Model):
    ...
    reportRead = models.BooleanField(default=False, blank=True, null=False)

    def __str__(self):
        return self.reportTitle + " (" + self.reportTitle + ")"

    def get_absolute_url(self):
        return reverse('report_list', kwargs={'pk': self.pk})

    def get_total_unit(self):
        t = 0
        total = Report.objects.all()
        for i in total:
            t +=i
        return i
...
class Reg(models.Model):
    owner = models.CharField(max_length=300)
    is_reg = models.BooleanField(default=False, blank=True, null=False)
